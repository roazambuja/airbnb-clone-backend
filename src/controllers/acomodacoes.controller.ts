...
export async function listarAcomodacoes(req: Request, res: Response) {
  ...
  const query =
    startOfQuery !== -1 ? req.originalUrl.substring(startOfQuery + 1) : "";
  ...
  for (const key in queryParams) {
    ...
    if (key === "general") {
      ...
      for (const comodidade in queryParams[key]) {
        const numComodidade = Number.parseInt(
          queryParams[key]![comodidade],
          10,
        );
  ...
  try {
    let acomodacoesFiltradas = await AcomodacaoModel.find(
      filtroMongoose,
    ).exec();
    ...
    if (
      queryParams.hasOwnProperty("check-in") &&
      queryParams.hasOwnProperty("check-out")
    ) {
      acomodacoesFiltradas = acomodacoesFiltradas.filter(async (acomodacao) => {
        ...
        reservas.forEach((reserva) => {
          ...
          acomodacaoValida =
            (dataCheckIn < dataInicioReserva &&
              dataCheckOut < dataInicioReserva) ||
            (dataCheckIn > dataTerminoReserva &&
              dataCheckOut > dataTerminoReserva);
